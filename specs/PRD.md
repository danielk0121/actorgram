...
- 메뉴 항목: 배우, 영화, 사진 검색, AI 검색, 어바웃(? 버튼)
...
- 배우 이름 끝에 출연 작품 수를 `[n]` 형식으로 표시한다. 출연 작품이 1개인 경우 미표시.
...
- 각 배우 이름 옆에 해당 배우의 전체 출연 작품 수를 `[n]` 형식으로 표시한다.
...
- 상단바 우측의 ? 버튼을 클릭하면 어바웃 화면으로 이동한다.
- 서비스 개요, 저작권 고지, 오픈소스 라이선스 정보를 표시한다.
- 오픈소스 저작권 고지 누락 문제를 명시한다.
- 개발자 정보를 표시한다.
...
| 출생연도 | Birth Year | `birthYear` | 배우의 출생 연도. Actor의 속성. |
...
| 포스터 | Poster | `posterUrl` | 작품의 대표 포스터 이미지. Movie의 속성. |
| 연도 | Year | `year` | 작품의 개봉 연도. Movie의 속성. |
| 장르 | Genre | `genre` | 작품의 장르. Movie의 속성. |
| 감독 | Director | `director` | 작품의 감독. Movie의 속성. |
| 작가 | Writer | `writer` | 작품의 각본가. Movie의 속성. |
| 관람등급 | Age Rating | `ageRating` | 작품의 관람 가능 연령 등급. Movie의 속성. |
| 상영시간 | Runtime | `runtime` | 작품의 상영 시간 (분 단위). Movie의 속성. |
| 개봉일 | Release Date | `releaseDate` | 작품의 개봉 날짜. Movie의 속성. |
| 제작국가 | Country | `country` | 작품의 제작 국가. Movie의 속성. |
| 줄거리 | Overview | `overview` | 작품의 줄거리 텍스트. Movie의 속성. |
